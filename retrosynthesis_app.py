        ...
        while True:
            response = openai.ChatCompletion.create(
                ...
                messages=[
                    ...
                    {"role": "user", "content": f"product: {product}, Class: {reaction_class}"},
                ],
            )
            ...
            if is_valid(reactant):
                ...
                if is_commercially_known(reactant):
                    ...
                    print(f"Reactant '{reactant}' not commercially known in PubChem. Predicting again...")
                    product = reactant
            else:
                print(f"Invalid reactant '{reactant}'. Predicting again...")
        log_input_output(username, f"product: {product}, Class: {reaction_class}", ", ".join(reactants))
        return reactants
...
def verify_credentials(username, password):
    ...
    c.execute("SELECT * FROM registered_users WHERE username = ? AND password = ?", (username, password))
    result = c.fetchone()
    if result:
        return True
    else:
...
def register_user(username, password):
    ...
    if result:
        return False
    c.execute("INSERT INTO registered_users (username, password) VALUES (?, ?)", (username, password))
    conn.commit()
    return True
...
def log_login_time(username, status):
    ...
    c.execute("INSERT INTO login_records (username, login_time, status) VALUES (?, ?, ?)",
...
def log_input_output(username, input_text, output_text):
    ...
    current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    c.execute("INSERT INTO input_output_records (username, input_text, output_text, timestamp) VALUES (?, ?, ?, ?)",
...
def main():
    ...
    if st.button("Predict"):
        if not product or not reaction_class:
            ...
            reactants = generate_prediction(product, reaction_class, st.session_state.username)
            if reactants:
                ...
                if is_commercially_known(last_reactant):
                    st.write(f"Reactant {len(reactants)}: {last_reactant} (Commercially Known)")
                else:
                    st.write(f"Reactant {len(reactants)}: {last_reactant} (Not Commercially Known)")
            else:
